...
from resnet_dco import resnet34
import torchsummary
import gap as gap
from attention import ACmix, iAFF, AFF
...
class bistim(nn.Module):
    def __init__(self,num_class=1):
        super(bistim,self).__init__()
        
        out_planes = num_class*8
        self.backbone =resnet34(pretrained=True)
        # self.sde_module = SDE_module(512,512,out_planes)
        self.sde_module = SAA_module(512,512,out_planes)
        ...
        self.sci_network = sci_network()


    def forward(self, x):
        
        # ilist, rlist, inlist, attlist, single_r = self.sci_network(x)
        ilist, rlist, inlist, attlist = self.sci_network(x)
        ...
        print(c4.shape, r5.shape, d4.shape)
        # add vig block
        ...
        # print(mapped_c5.shape)

        # return cls_pred, mapped_c5
        return cls_pred, mapped_c5, ilist, rlist, inlist, attlist
        # return cls_pred, mapped_c5, ilist, rlist, inlist, attlist, single_r
...
# Structure Aware Activation (SAA)
class SAA_module(nn.Module):
    def __init__(self, in_channels, out_channels, num_class):
        ...
        self.att1 = ACmix(in_planes=self.inter_channels, out_planes=self.inter_channels)
        self.att2 = ACmix(in_planes=self.inter_channels, out_planes=self.inter_channels)
        self.att3 = ACmix(in_planes=self.inter_channels, out_planes=self.inter_channels)
        self.att4 = ACmix(in_planes=self.inter_channels, out_planes=self.inter_channels)
        self.att5 = ACmix(in_planes=self.inter_channels, out_planes=self.inter_channels)
        self.att6 = ACmix(in_planes=self.inter_channels, out_planes=self.inter_channels)
        self.att7 = ACmix(in_planes=self.inter_channels, out_planes=self.inter_channels)
        self.att8 = ACmix(in_planes=self.inter_channels, out_planes=self.inter_channels)
        ...
        # 
        self.conv8 = nn.Sequential(nn.Dropout2d(0.1, False), nn.Conv2d(self.inter_channels, self.inter_channels, 1))
        
        # self.iaff = iAFF(channels=64)
        self.iaff = AFF(channels=64)
        
        self.expand_layer = nn.Conv2d(64, 512, kernel_size=1)
    ...
    def forward(self, x, d_prior):
        ...
        feat1 = self.conv8(self.att1(x[:,:self.inter_channels]) * F.sigmoid(enc_feat[:,0:self.inter_channels]))
        feat2 = self.conv8(self.att2(x[:,self.inter_channels:2*self.inter_channels]) * F.sigmoid( enc_feat[:,self.inter_channels:2*self.inter_channels]))
        feat3 = self.conv8(self.att1(x[:,2*self.inter_channels:3*self.inter_channels]) * F.sigmoid(enc_feat[:,2*self.inter_channels:3*self.inter_channels]))
        feat4 = self.conv8(self.att1(x[:,3*self.inter_channels:4*self.inter_channels]) * F.sigmoid( enc_feat[:,3*self.inter_channels:4*self.inter_channels]))
        feat5 = self.conv8(self.att1(x[:,4*self.inter_channels:5*self.inter_channels]) * F.sigmoid(enc_feat[:,4*self.inter_channels:5*self.inter_channels]))
        feat6 = self.conv8(self.att1(x[:,5*self.inter_channels:6*self.inter_channels]) * F.sigmoid(enc_feat[:,5*self.inter_channels:6*self.inter_channels]))
        feat7 = self.conv8(self.att1(x[:,6*self.inter_channels:7*self.inter_channels]) * F. sigmoid(enc_feat[:,6*self.inter_channels:7*self.inter_channels]))
        feat8 = self.conv8(self.att1(x[:,7*self.inter_channels:8*self.inter_channels]) * F.sigmoid(enc_feat[:,7*self.inter_channels:8*self.inter_channels]))

        feat = self.iaff(feat1, feat2)
        feat = self.iaff(feat, feat3)
        feat = self.iaff(feat, feat4)
        feat = self.iaff(feat, feat5)
        feat = self.iaff(feat, feat6)
        feat = self.iaff(feat, feat7)
        feat = self.iaff(feat, feat8)
        
        feat = self.expand_layer(feat)
...
class DANetHead(nn.Module):
    def __init__(self, in_channels, inter_channels, norm_layer=nn.BatchNorm2d):
        ...
        # not work
        # self.sa = ACmix(in_planes=inter_channels, out_planes=inter_channels)
        # self.sc = ACmix(in_planes=inter_channels, out_planes=inter_channels)
        # not work 
        # self.sa = ParallelPolarizedSelfAttention(channel=inter_channels)
        # self.sc = SequentialPolarizedSelfAttention(channel=inter_channels)
        # 
        self.sa = HaloAttention(dim=inter_channels, block_size=2, halo_size=1)
        self.sc = HaloAttention(dim=inter_channels, block_size=2, halo_size=1)
...
if __name__ == '__main__':
     ...
     random_array = np.random.rand(1, 3, 512, 512)
     input_tensor = torch.tensor(random_array, dtype=torch.float32)
